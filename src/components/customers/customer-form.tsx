...
export function CustomerForm({ action }: Readonly<{ action: CustomerFormAction }>) {
  ...
  function selectCompany(result: RegistryCompany) {
  ...
  return (
    <form action={formAction} className="space-y-5" ref={formRef}>
      <div className="grid gap-5 sm:grid-cols-2">
        <label className="text-sm font-medium text-[#19332d] sm:col-span-2">Firmanavn<CompanySearchField onSelect={selectCompany} />{state.fieldErrors?.companyName ? <span className="mt-1 block text-xs font-normal text-[#a63e2a]">{state.fieldErrors.companyName[0]}</span> : null}</label>
